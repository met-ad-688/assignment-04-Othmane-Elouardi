...
# --- Interactive Diagnostic Plots (Plotly) for Spark Regression ---

# CONFIG: set this to your label column
label_col = label_col  # e.g., "label" or "label_log"
...
# 1) Get predictions & residuals safely from Spark
#    - Drop rows with null/NaN/Inf labels or predictions
#    - Downsample to keep the page responsive
spark_preds = (
    model.transform(df_fe)
         .select(label_col, "prediction")
         .dropna(subset=[label_col, "prediction"])
)

# Filter out non-finite labels/preds (Spark side)
from pyspark.sql import functions as F
finite = (
    spark_preds
      .where(~F.isnan(F.col(label_col)))
      .where(~F.isnan(F.col("prediction")))
)

# Limit rows for interactivity (tune as needed)
MAX_ROWS = 50_000
n_rows = finite.count()
if n_rows > MAX_ROWS:
    frac = MAX_ROWS / n_rows
    finite = finite.sample(withReplacement=False, fraction=frac, seed=42)

pdf = finite.toPandas()
if pdf.empty:
    raise ValueError(
        "No rows available for diagnostics after filtering. "
        "Check that your model produced predictions and label_col exists."
    )

# 2) Compute residuals and standardized residuals
pdf["residual"] = pdf[label_col] - pdf["prediction"]
resid = pdf["residual"].values
pred  = pdf["prediction"].values
...
# 3) QQ data (theoretical normal quantiles vs ordered standardized residuals)
n = len(z)
# Use plotting positions for better tails
pp = (np.arange(1, n + 1) - 0.5) / n
theoretical = norm.ppf(pp)
ordered = np.sort(z)

# Reference line for QQ (fit a simple line through quartiles)
x1, x3 = np.percentile(theoretical, [25, 75])
y1, y3 = np.percentile(ordered, [25, 75])
slope = (y3 - y1) / (x3 - x1) if x3 != x1 else 1.0
intercept = y1 - slope * x1
qq_line_x = np.array([theoretical.min(), theoretical.max()])
qq_line_y = intercept + slope * qq_line_x

# 4) Build interactive 2×2 dashboard
fig = make_subplots(
    rows=2, cols=2,
    subplot_titles=(
        "Predicted vs Actual",
...
fig.add_trace(
    go.Scattergl(
        x=pred, y=y, mode="markers", opacity=0.5,
        marker=dict(size=5),
...
# 45° line
lb = float(np.nanmin([pred.min(), y.min()]))
ub = float(np.nanmax([pred.max(), y.max()]))
fig.add_trace(
    go.Scatter(
        x=[lb, ub], y=[lb, ub],
        mode="lines", line=dict(dash="dash"),
        name="45° line", showlegend=False
    ),
    row=1, col=1
)
...
fig.add_trace(
    go.Scattergl(
        x=pred, y=resid, mode="markers", opacity=0.5, marker=dict(size=5),
...
fig.add_trace(
    go.Histogram(
        x=resid, nbinsx=60, histnorm="probability",
        hovertemplate="Residual=%{x:.3f}<br>Prob=%{y:.3f}<extra></extra>"
    ),
    row=2, col=1
)
...
fig.add_trace(
    go.Scattergl(
        x=theoretical, y=ordered, mode="markers", opacity=0.6, marker=dict(size=5),
...
fig.add_trace(
    go.Scatter(
        x=qq_line_x, y=qq_line_y, mode="lines",
...
fig.update_layout(
    ...
    title=f"Interactive Diagnostic Plots ({label_col})",
    template="plotly_white",
    showlegend=False
)

# Display inline + save a standalone HTML for the report
display(fig)

os.makedirs("_output", exist_ok=True)
html_path = "_output/diagnostics_interactive.html"
fig.write_html(html_path, include_plotlyjs="cdn", full_html=True)
display(HTML(f'<p>🔗 <b>Saved</b>: <a href="{html_path}" target="_blank">{html_path}</a></p>'))
